...
export async function middleware(request: NextRequest) {
  ...
  // Skip auth checks if Supabase is not configured yet (local dev without .env)
  ...
  if (!user && isDashboard) return NextResponse.redirect(new URL("/auth/login", request.url));
