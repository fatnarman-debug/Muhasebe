...
type Inv = {
  id: string; invoice_number: string; status: string; invoice_date: string; due_date: string;
  ocr_number: string | null; subtotal: number; vat_amount: number; total: number;
  your_reference: string | null; notes: string | null;
...
export default function KonsultInvoiceDetail() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setMsg("Fakturan skickades till kunden.");
      await load();
    } catch (e) {
  ...
  return (
    <div style={{ padding: "24px 32px 48px", maxWidth: 860, margin: "0 auto" }}>
      <Link href="/konsult/fakturor" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-4">
      ...
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ padding: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, borderBottom: "1px solid #f3f4f6" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: 12, color: "#6b7280" }}>Fakturadatum: <b>{inv.invoice_date}</b></p>
            <p style={{ fontSize: 12, color: "#6b7280" }}>Förfaller: <b>{inv.due_date}</b></p>
            {inv.ocr_number && <p style={{ fontSize: 12, color: "#6b7280" }}>OCR: <b>{inv.ocr_number}</b></p>}
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", padding: 16 }}>
          <div style={{ width: 260 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, paddingTop: 8, borderTop: "1px solid #e5e7eb", fontSize: 15, fontWeight: 800, color: "#111827" }}><span>Att betala</span><span>{formatSEK(inv.total)}</span></div>
          </div>
        </div>
      </div>
    </div>
