    ...
    (async () => {
      try {
        ...
        const { data, error: err } = await supabase
          .from("invoices")
          .select("id, invoice_number, status, invoice_date, due_date, total, customers(name), client_companies(name)")
    ...
    <>
      <header className="sticky top-0 z-40 flex items-center justify-between px-8"
        ...
        <div className="flex items-center gap-3">
          ...
          <p style={{ fontSize: 13, color: "#9ca3af" }}>{list.length} fakturor</p>
        </div>
        <button onClick={() => router.push("/konsult/fakturor/new")}
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", overflow: "hidden" }}>
          {loading ? (
            ...
            <div style={{ padding: 48, textAlign: "center", color: "#9ca3af", fontSize: 13 }}>
              Inga fakturor ännu. Skapa din första med “Ny faktura”.
            </div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #f3f4f6" }}>
                  {["Fakturanr", "Klientföretag", "Kund", "Förfaller", "Status", "Belopp"].map((h) => (
